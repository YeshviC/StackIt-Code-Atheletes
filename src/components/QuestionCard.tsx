...
const QuestionCard = ({ ques }: { ques: Models.Document }) => {
  ...
  React.useEffect(() => {
    ...
    // Dynamic values
    setRelativeTime(convertDateToRelativeTime(new Date(ques.$createdAt)));
    setAvatarUrl(avatars.getInitials(ques.author.name, 24, 24).href);
  }, [ques.$createdAt, ques.author.name]);
    ...
    >
      ...
      <div className="relative w-full">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-3 text-sm">
          ...
          {relativeTime && <span>asked {relativeTime}</span>}
        </div>
      </div>
    </div>
  );
